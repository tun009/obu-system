    ...
    return (
        <div className="flex flex-col h-full bg-white border-r border-gray-200">
            <div className="p-5 pb-0">
                <h2 className="text-xl font-bold text-gray-800 tracking-tight">{t('vehicleList.title')} ({vehicles.length})</h2>
                ...
                <div className="grid grid-cols-5 gap-2 mt-4 text-center">
                    <div className="bg-gray-50 rounded-lg p-2 border border-gray-100 cursor-pointer hover:bg-blue-50" onClick={() => setFilter('ALL')}>
                        <p className="text-[12px] font-semibold text-gray-500">{t('vehicleList.totalVehicles', 'Tổng xe')}</p>
                    ...
                    <div className="bg-green-50 rounded-lg p-2 border border-green-100 cursor-pointer hover:bg-green-100" onClick={() => setFilter('RUNNING')}>
                        <p className="text-[12px] font-semibold text-gray-500 line-clamp-1">{t('monitor.statusRunning')}</p>
                    ...
                    <div className="bg-orange-50 rounded-lg p-2 border border-orange-100 cursor-pointer hover:bg-orange-100" onClick={() => setFilter('STOPPED')}>
                        <p className="text-[12px] font-semibold text-gray-500 line-clamp-1">{t('monitor.statusStopped')}</p>
                    ...
                    <div className="bg-gray-100/50 rounded-lg p-2 border border-gray-200 cursor-pointer hover:bg-gray-200" onClick={() => setFilter('PARKED')}>
                        <p className="text-[12px] font-semibold text-gray-500 line-clamp-1">{t('monitor.statusParked')}</p>
                    ...
                    <div className="bg-red-50 rounded-lg p-2 border border-red-100 cursor-pointer hover:bg-red-100" onClick={() => setFilter('LOST_SIGNAL')}>
                        <p className="text-[12px] font-semibold text-gray-500 line-clamp-1">{t('monitor.statusOffline')}</p>
                ...
                <div className="flex gap-2 mt-4 pb-4 overflow-x-auto no-scrollbar border-b border-gray-100">
                    <button className={`whitespace-nowrap px-3 py-1 rounded-full text-xs font-medium border ${filter === 'ALL' ? 'bg-[#335ddc]/10 text-[#335ddc] border-[#335ddc]/30' : 'text-gray-600 border-gray-200 bg-white hover:bg-gray-50'}`} onClick={() => setFilter('ALL')}>{t('monitor.statusAll', 'Tất cả')}</button>
                    <button className={`whitespace-nowrap px-3 py-1 rounded-full text-xs font-medium border ${filter === 'RUNNING' ? 'bg-green-100 text-green-700 border-green-300' : 'text-gray-600 border-gray-200 bg-white hover:bg-gray-50'}`} onClick={() => setFilter('RUNNING')}>{t('monitor.statusRunning')}</button>
                    <button className={`whitespace-nowrap px-3 py-1 rounded-full text-xs font-medium border ${filter === 'STOPPED' ? 'bg-orange-100 text-orange-700 border-orange-300' : 'text-gray-600 border-gray-200 bg-white hover:bg-gray-50'}`} onClick={() => setFilter('STOPPED')}>{t('monitor.statusStopped')}</button>
                    <button className={`whitespace-nowrap px-3 py-1 rounded-full text-xs font-medium border ${filter === 'PARKED' ? 'bg-gray-200 text-gray-700 border-gray-400' : 'text-gray-600 border-gray-200 bg-white hover:bg-gray-50'}`} onClick={() => setFilter('PARKED')}>{t('monitor.statusParked')}</button>
                    <button className={`whitespace-nowrap px-3 py-1 rounded-full text-xs font-medium border ${filter === 'ENGINE_ON' ? 'bg-blue-100 text-blue-700 border-blue-300' : 'text-gray-600 border-gray-200 bg-white hover:bg-gray-50'}`} onClick={() => setFilter('ENGINE_ON')}>{t('vehicleList.engineOn', 'Nổ máy')}</button>
                    <button className={`whitespace-nowrap px-3 py-1 rounded-full text-xs font-medium border ${filter === 'ENGINE_OFF' ? 'bg-red-100 text-red-700 border-red-300' : 'text-gray-600 border-gray-200 bg-white hover:bg-gray-50'}`} onClick={() => setFilter('ENGINE_OFF')}>{t('vehicleList.engineOff', 'Tắt máy')}</button>
                </div>
            </div>

            <div className="grid grid-cols-7 gap-2 px-5 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider bg-gray-50/50">
                <div className="col-span-3">{t('vehicleList.colPlate')}</div>
                <div className="col-span-2 text-center">{t('vehicleList.colStatus')}</div>
                <div className="text-center">{t('common.kmh', 'Km/h')}</div>
            ...
            <div className="flex-1 overflow-y-auto w-full pb-4">
                {filteredVehicles.map((v) => {
                    ...
                    >
                        ...
                        <p className="text-[11px] text-gray-400 mt-1 line-clamp-1">
                            ...
                            {lastUpdateStr && <span> · {t('monitor.lastUpdate')} {lastUpdateStr}</span>}
                        </p>
                    </div>
                    );
                })}
                ...
                {filteredVehicles.length === 0 && (
                    <div className="p-8 text-center bg-gray-50 h-full">
                        ...
                        <p className="text-gray-500 font-medium">{t('vehicleList.noVehiclesFound', 'Không tìm thấy xe nào phù hợp!')}</p>
                    </div>
                )}
            </div>
        </div>
