        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[9999]">
            <div className="bg-white rounded-xl shadow-2xl w-[600px] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    <h2 className="text-xl font-bold text-gray-800">{isEditMode ? t('addVehicle.titleEdit') : t('addVehicle.titleAdd')}</h2>
                ...
                <div className="p-6 pb-2">
                    <div className="grid grid-cols-2 gap-6 mb-6">
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-1.5">{t('common.vehicleType', 'Loại xe')} <span className="text-red-500">*</span></label>
                            <Input value={type} onChange={(e) => setType(e.target.value)} placeholder={t('common.vehicleTypePlaceholder', 'Nhập loại xe...')} className="w-full bg-gray-50/50" />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-1.5">{t('addVehicle.plate')} <span className="text-red-500">*</span></label>
                            <Input value={plate} onChange={(e) => setPlate(e.target.value)} placeholder={t('addVehicle.platePlaceholder')} className="w-full bg-gray-50/50" />
                        </div>
                    </div>

                    <div className="mb-6">
                        <label className="block text-sm font-semibold text-gray-700 mb-1.5">{t('addVehicle.imei')}</label>
                        <div className="flex gap-3">
                            <div className="flex-1 relative">
                                <Input
                                    value={imei}
                                    onChange={(e) => { setImei(e.target.value); setCheckStatus('idle'); }}
                                    placeholder={t('addVehicle.imeiPlaceholder')}
                                ...
                                />
                                {checkStatus === 'success' && !isEditMode && <p className="text-xs text-green-600 font-medium italic mt-1.5 px-0.5">{t('addVehicle.checkSuccess', 'Kết nối thiết bị thành công!')}</p>}
                                {checkStatus === 'error' && !isEditMode && <p className="text-xs text-red-500 font-medium italic mt-1.5 px-0.5">{t('addVehicle.checkError', 'IMEI không hợp lệ hoặc thiết bị đang off!')}</p>}
                            </div>
                            {!isEditMode && (
                                <Button variant="primary" onClick={handleCheck} className="h-10 px-5 shadow-sm font-medium">
                                    {t('addVehicle.checkButton', 'Kiểm tra')}
                                </Button>
                            )}
                        </div>
                    </div>
                    ...
                    {checkStatus === 'success' && !isEditMode && checkData && (
                        <div className="mb-4 bg-green-50/50 border border-green-100 p-4 rounded-lg text-sm text-gray-700">
                            <p className="font-semibold text-gray-800 mb-2">{t('addVehicle.deviceInfo', 'Thông tin thiết bị từ hệ thống:')}</p>
                            <ul className="list-disc pl-5 space-y-1 text-gray-600">
                                <li>GPS: <span className="font-mono text-xs">Lat {checkData.lat.toFixed(5)} - Long {checkData.lng.toFixed(5)}</span></li>
                                <li>{t('history.speed', 'Vận tốc')}: <span className="font-medium text-gray-800">{Math.round(checkData.speed || 0)} {t('common.kmh', 'km/h')}</span></li>
                                <li>{t('vehicleList.colUpdateTime', 'Cập nhật cuối')}: <span className="text-gray-800 font-medium">{new Date(checkData.lastUpdate).toLocaleTimeString()}</span></li>
                                <li>{t('history.status', 'Tình trạng')}: <span className="text-green-600 font-medium">{t('addVehicle.dataStable', 'Đang đổ dữ liệu ổn định')}</span></li>
                            </ul>
                        </div>
                    )}
                </div>
                ...
                <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 bg-white">
                    ...
                    <Button variant="primary" onClick={handleSubmit} className="px-6 shadow-sm transition-colors">
                        {isEditMode ? t('addVehicle.updateButton', 'Cập nhật') : t('addVehicle.save')}
                    </Button>
                </div>
            </div>
        </div>
